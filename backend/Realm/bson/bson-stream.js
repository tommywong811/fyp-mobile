...
/**
 * This is a modified version of Tim Kuijsten's
 * [bson-stream module](https://www.npmjs.com/package/bson-stream),
 * where I have added the ability to configure the inner call of 
 * `BSON.deserialize`.
 */
...
* bsonOptions {Object, default {promoteBuffers: true}} The options fed into the inner call of `bson.deserialize`
*/
function BSONStream(opts) {
  ...
  if (typeof opts.bsonOptions !== 'undefined' && typeof opts.bsonOptions !== 'object') { throw new TypeError('opts.bsonOptions must be an object'); }
  ...
  this._bsonOptions = opts.bsonOptions || {};
  this._bsonOptions.promoteBuffers = true;
...
BSONStream.prototype._parseDocs = function _parseDocs(cb) {
  ...
  try {
    obj = BSON.deserialize(rawdoc, this._bsonOptions);
  } catch (err) {
    // discard buffer
    this._reset();
    cb(err);
